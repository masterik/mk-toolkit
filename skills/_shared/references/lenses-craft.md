...
Five reading stances: `architecture`, `quality`, `tests`, `docs`, `comments`. A lens is **what a
reviewer goes looking for** — it decides where to look and how hard to push, never what severity
to give (that is `review-severity.md`, and it is the same bar for every lens).

This set goes to the **Claude subagent reviewer** in `review-changes` — promptable, and it needs
repo context the other reviewers do not gather. The correctness set (`bugs`, `impl`,
`adversarial`) is in `lenses-correctness.md` and goes to a different reviewer; you do not carry
