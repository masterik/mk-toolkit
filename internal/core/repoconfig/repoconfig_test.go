...
// The shadowed-config sentence is the one deliberate exception to "one producer per
// degradation sentence": it exists in Go and in lib/common.sh, because `mkit init`
// must refuse a shadowed path before it has located a payload, and a remedy that is
// unavailable exactly when the payload is missing is not a remedy.
//
// An exception that is only documented is an exception that drifts. This compares the
// two implementations byte for byte over every rule shape, so a reword on either side
// fails here rather than in a user's terminal months later.
func TestRemedyParityWithShell(t *testing.T) {
	common, err := filepath.Abs(filepath.Join("..", "..", "..", "plugin", "scripts", "lib", "common.sh"))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(common); err != nil {
		t.Skipf("payload not beside the work tree: %v", err)
	}
	...
	for _, rule := range []string{
		...
		t.Run(rule, func(t *testing.T) {
			...
			cmd := exec.Command("bash", "-c", ". "+common+"; mkit_config_ignored_remedy")
			cmd.Dir = dir
			cmd.Env = append(os.Environ(),
				"GIT_CONFIG_GLOBAL=/dev/null", "GIT_CONFIG_SYSTEM=/dev/null", "GIT_CONFIG_NOSYSTEM=1")
			out, err := cmd.Output()
			if err != nil {
				t.Fatalf("mkit_config_ignored_remedy: %v", err)
			}

			shell := strings.TrimRight(string(out), "\n")
			if got := ShadowedRemedy(st); got != shell {
				t.Errorf("the two producers disagree for rule %q:\n  go:    %s\n  shell: %s",
					rule, got, shell)
			}
		})
	}
